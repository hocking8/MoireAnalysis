...
def estimate_tip_velocity(
    ...
    for channel_name in channel_names:
        ...
        # print(f"\nSensor: {channel_name}")
        # print(f"v_fast: {v_fast:.2f} +/- {v_fast_std:.2f}")
        # print(f"v_slow: {v_slow:.2f} +/- {v_slow_std:.2f}")
        # print(f"scan_distance: {scan_distance:.2f} +/- {scan_distance_std:.2f}")
...
def extract_peaks(
    ...
    for channel_name in channel_names:
        ...
        # all_peaks = []
        # for peak in peaks:
        #     # get the 6 closest peaks to the current peak
        #     peak_distances = np.linalg.norm(peaks - peak, axis=1)
        #     peak_distances = np.argsort(peak_distances)
        #     closest_peaks = peaks[peak_distances[1:7]]
        #     closest_peak_distances = np.linalg.norm(closest_peaks - peak, axis=1)
        #     peak_std = np.std(closest_peak_distances, ddof=1)
        #     if peak_std > 2:
        #         continue
        ...
        # the error of the peaks can be estimated by looking at the pixels
        # I.e. The error of the peak position can be estimated by position +/- 0.5 pixel
        # as the peak position is distributed uniformly in the pixel
        # the standard deviation of the peak position is then 1 / sqrt(12) = 0.5 / sqrt(3)
        # See https://www.isobudgets.com/probability-distributions-for-measurement-uncertainty/
...
def calculate_maximum_overlap_rectangle(
    ...
    def find_optimal_rectangle(
        ...
        # this means we need to find the biggest x and smallest y value (0, 1)
        # then the biggest x and biggest y value (0, 0)
        # then the smallest x and biggest y value (1, 0)
        # then the smallest x and smallest y value (1, 1)
...
def estimate_drift_from_cc(
    ...
    for scan_collection in [UP_scans, DOWN_scans]:
        for index, scan in enumerate(scan_collection):
            ...
            # get the peak that is closest to the center
        ...
        #     # get the middle peak
        #     plt.axvline(x=data_1.shape[1] / 2, color="r", linestyle="--")
        #     plt.axhline(y=data_1.shape[0] / 2, color="r", linestyle="--")
        #     plt.scatter(peaks[:, 1], peaks[:, 0], c="r", s=50, marker="x")
        #     plt.scatter(center_peak[1], center_peak[0], c="b", s=50, marker="x")

        # plt.xlim(data_1.shape[1] / 2 - 50, data_1.shape[1] / 2 + 50)
        # plt.ylim(data_1.shape[0] / 2 - 50, data_1.shape[0] / 2 + 50)
        # plt.show()
